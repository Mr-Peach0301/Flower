...
class D3Dataset(Dataset):
    def __init__(self, train_file, tokenizer, max_len=2048, sample=-1, test = False, seed=0, category="", K=4, weight_dict=None, group_title=None, dedup=False):
...
{data_point["output"]}"""
    ...
    def get_history(self, row):
        ...
        return {"input": f"The user has enjoyed the following {self.category}s before: {history}",
                "output": target_item + '\n',
...
"""
        ...
        flag = 0
        for i in self.group_title.keys():
            if target_item.strip('\n') in self.group_title[i]:
                flag=1
                weight = self.weight_dict[i]
        if flag==0:
            weight = self.weight_dict['7']
