...
def score_fast(
    ...
    def normalize_element_mid(tensor, a, b):
        ...
        normalized_value = (element_b - m) / (max_val.item() - min_val.item()) + 0.5
    ...
    for index, input_ids in enumerate(encoded_input[:, skip_first:]):
        ...
        for j in group_title.keys():
            if decoded_text in group_title[j]:
                if sasrec_logits is not None:
                    sasrec_p.append(normalize_element(sasrec_logits, train_index[index], title2id[decoded_text]))
                tag.append(j)
                break
        ...
        for j in range(token_ids.shape[1]):
            ...
            if j==0:
                if str(token) in token_distri['first']:
                    ...
                    if sasrec_logits is not None:
                        logr[i, j, token] += np.log(sasrec_p[i] + 1e-20)*S_lamda
                else:
                    logr[i, j:, :] = -15
                ...
                if get_hash(token_ids[i, :j, 0].tolist()) in token_distri and str(token) in token_distri[get_hash(token_ids[i, :j, 0].tolist())]:
                    ...
                    if token_ids[i, j-1, 0].item()==termination_token_id and tokenizer.decode(token_ids[i, :j-1, 0].tolist()) in possible_output:
                        ...
                        term_prob.append(token_distri[get_hash(token_ids[i, :j-1, 0].tolist())][str(termination_token_id)]+np.log(sasrec_p[i] + 1e-20)*S_lamda)
                    else:
                        complete_title.append(-1)
                        term_prob.append(-15)
                    break
            if j==token_ids.shape[1]-1 and token!=termination_token_id:
                if tokenizer.decode(token_ids[i, :, 0].tolist()) in possible_output:
                    ...
                    if sasrec_logits is None:
                        ...
                        term_prob.append(token_distri[get_hash(token_ids[i, :, 0].tolist())][str(termination_token_id)]+np.log(sasrec_p[i] + 1e-20)*S_lamda)
                else:
                    complete_title.append(-1)
                    term_prob.append(-15)
                ...
                if sasrec_logits is None:
                    ...
                    term_prob.append(token_distri[get_hash(token_ids[i, :j, 0].tolist())][str(termination_token_id)]+np.log(sasrec_p[i] + 1e-20)*S_lamda)
