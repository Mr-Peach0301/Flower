...
from utils_logp_add_logs import (
...
class EvaluateCallback(TrainerCallback):
    def __init__(self, test_data_path, category, info_file, group_title_path, tokenizer, temperature, logits_file=None, title_token_distribution=None, possible_output=None, gfn_theta=0.9, train_theta=0.999):
        ...
        self.path = f'./temp_sft&gfn_logp_add_logs/{self.category}_base/final_result.json'
        self.result_data = f'./temp_sft&gfn_logp_add_logs/{self.category}_base'
...
{_}''' for _ in info]
            ...
            gao(self.path, self.info_file, self.result_data, self.group_title_path, self.category, str(round(int(epoch))))

class MyTrainer(Trainer):
    def __init__(self, tokenizer, min_sentence_len, max_sentence_len, group_title_path, title_token_distribution, possible_output, gfn_theta, train_theta, sasrec_logits, info_file, S_lamda, *args, **kwargs):
        ...
        self.gfn_theta = gfn_theta
        self.train_theta = train_theta
        self.sasrec_logits = sasrec_logits
    
    def gfn_forward(self, prompt, model, pf_temperature=1.0, mode='train', action_seq=None, sasrec_logits=None):
        reward_fn = partial(
            self.reward.score,
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        generated_text, log_pf, log_pterm, log_r, log_r_unpenalized = self.gfn_forward(
            inputs, model=model, action_seq=None, mode='train', sasrec_logits=self.sasrec_logits
        )
