...
from dataset_reweight import D3Dataset
...
class EvaluateCallback(TrainerCallback):
    def __init__(self, test_data_path, category, info_file, group_title_path, tokenizer, temperature, weight_dict, logits_file=None, title_token_distribution=None, possible_output=None):
        ...
        with open(group_title_path, "r") as input_file:
            self.group_title = json.load(input_file)
        self.tokenizer = tokenizer
        self.weight_dict = weight_dict
        self.path = f'./temp_sft_item_side_reweight/{self.category}_base/final_result'
        self.result_data = f'./temp_sft_item_side_reweight/{self.category}_base'
...
{_}''' for _ in info]
            ...
            val_dataset=D3Dataset(train_file=self.test_data_path, tokenizer=tokenizer,max_len=2560, category=category, test=True,K=0, weight_dict=self.weight_dict, group_title=self.group_title)
...
class MyTrainer(Trainer):
    def __init__(self, tokenizer, min_sentence_len, max_sentence_len, group_title_path, title_token_distribution, possible_output, *args, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        sft_input = {k:v for k,v in inputs.items() if k not in ['gfn_input','index','weight']}
        ...
        loss = torch.mean(weight * torch.mean(loss_fct(shift_logits, shift_labels).view(weight.shape[0], -1)))
...
class CustomDataCollator(transformers.DataCollatorForSeq2Seq):
    def __call__(self, features):
        ...
        for i in range(len(features)):
            ...
            weight.append(features[i]['weight'])
        ...
        features = [{k:v for k,v in f.items() if k not in ['gfn_input','index', 'weight']} for f in features]
...
def train(
    ...
    gh_tr = [0]*len(group_title)
    gh_ta = [0]*len(group_title)
    train_d = pd.read_csv(train_file)
    for i in tqdm(range(len(train_d))):
        row = train_d.iloc[i]
        history_item_title = eval(row['history_item_title']) 
        for j in history_item_title:
            for k in group_title.keys():
                if j in group_title[k]:
                    gh_tr[int(k)] += 1
        target_item = str(row['item_title'])
        for k in group_title.keys():
            if target_item in group_title[k]:
                gh_ta[int(k)] += 1
    gh_tr = [x/sum(gh_tr) for x in gh_tr]
    gh_ta = [x/sum(gh_ta) for x in gh_ta]
    
    weight_dict = {}
    for i in group_title.keys():
        weight_dict[i] = gh_tr[int(i)] / gh_ta[int(i)]
    ...
    train_data = D3Dataset(train_file=train_file, tokenizer=tokenizer, max_len=cutoff_len,  sample=sample, seed=seed, category=category, K = K, weight_dict=weight_dict, group_title=group_title)
    val_data = D3Dataset(train_file=eval_file, tokenizer=tokenizer, max_len=cutoff_len,  sample=sample, category=category, K = K, weight_dict=weight_dict, group_title=group_title)
