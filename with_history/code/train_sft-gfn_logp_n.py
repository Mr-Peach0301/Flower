...
class EvaluateCallback(TrainerCallback):
    ...
    def perform_evaluation(self, args, state, control, step_counter, epoch_or_step, **kwargs):
        with torch.no_grad():
...
{_}''' for _ in info]
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if self.step_counter % self.eval_steps == 0:
            self.perform_evaluation(args, state, control, self.step_counter, 'step', **kwargs)

    def on_epoch_end(self, args, state, control, **kwargs):
        self.perform_evaluation(args, state, control, self.step_counter, 'epoch', **kwargs)
